...
const SliderPlaybarFul = ({ progress, duration }) => {
  ...
  const width = windowDimensions.width; // Meme valeur que dans le css(width: 95vw)
  const heightSlider = 40; // Meme valeur que dans le css(height: 20px)
  const widthSliderknob = 30;
  const radius = width / 2;
  const radiusCircle = window.innerWidth * 1.5;
  ...
  const thumbContainerStyle = {
    transform: `translateY(calc(${
      -opposite +
      width -
      heightSlider / 2 + widthSliderknob/2 +
      (((Math.abs(value - 50) / 100) * (width + heightSlider * 50 + widthSliderknob*25 -20*100)) / 100 )
    }px  - 100%
      )) translateX(calc(${
  ...
  const sliderStyle = {
    transform: `translateY(calc(${
      ...
      width * 0.85 -
      heightSlider*1.5 -
      (((Math.abs(value - 50) / 100) * (width + heightSlider*100)) / 100 )

      // - Math.abs((value / 100 )-50) * heightSlider
    }px - 100%)) translateX(-50%)`,
  };
